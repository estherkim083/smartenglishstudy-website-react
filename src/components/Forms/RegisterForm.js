...
function RegisterForm(props) {
  ...
  const KakaoRegister= () => {
    fetch(`https://kauth.kakao.com/oauth/token`, {
      ...
      body: `grant_type=authorization_code&client_id=${process.env.REACT_APP_KAKAO_REST_API_KEY}&redirect_uri=http://localhost:3000/auth/register/kakao&code=${CODE}`,
    })
      .then(res=> res.json())
      .then(data => {
  ...
  const KAKAO_AUTH_URL= `https://kauth.kakao.com/oauth/authorize?client_id=${process.env.REACT_APP_KAKAO_REST_API_KEY}&redirect_uri=http://localhost:3000/auth/register/kakao&response_type=code`;
  ...
  const handleNaverRegister= ()=> {
    window.location.href=`https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=4N4hr24Can8FuT0yjggz&redirect_uri=http://localhost:3000/auth/register/naver&state=sdfkjashftreer`;
