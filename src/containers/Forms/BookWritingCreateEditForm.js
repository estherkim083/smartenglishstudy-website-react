...
function BookWritingCreateEditForm(props) {
  ...
  const [bookInfoVal, setBookInfoVal] = useState(null);
  ...
  const bookInfoInputValChange = (e)=> {
    setBookInfoVal(e.target.value);
  }
  ...
  const btnSubmit= async() => {
    ...
    if(id!= null || id!== undefined) {
      ...
      axios
          .post(baseURL+"writing/create-book-writing-room/" , {            
            topic: topicVal,
            about_content: txt,         
            about_room: aboutRoomVal,
            room_title: titleVal,        
      ...
      > 
        ...
        <Grid item xs={12}>
          <div style= {{display: 'flex', flexDirection: 'row', marginTop: "0"}}>
              ...
              <Input
              multiline 
              placeholder="글쓰기 영어책에 관해서."
              inputProps={{
                  'aria-label': 'Description',
              }}
              style={{margin: "20px", marginTop: "-13px", width: "1200px", height: '125px'}}
              value={bookInfoVal}
              onChange={bookInfoInputValChange}
              minRows= {5}
              />
          </div>
        </Grid>
...
BookWritingCreateEditForm.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(BookWritingCreateEditForm);
