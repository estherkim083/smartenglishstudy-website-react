...
function EssayWriting(props) {
  ...
  const dataJson = useFirestoreQuery(doc(db, "comment", docId));
  ...
  useEffect(() => {
    if (isLoaded) {
      axios
        .get(baseURL + "writing/define-essay-editor/" + id + "/" + userid, {
          //첨삭가능한 친구 목록 불러오기.
          headers: {
      ...
      getDoc(doc(db, "current_user_data", localStorage.getItem("email"))).then(
        (docSnap) => {
          if (docSnap.exists()) {
      ...
      axios
        .get(
          baseURL +
            "writing/check-if-essay-participant-user/" +
            id +
            "/" +
            userid,
          {
        ...
        .then((res) => {
          ...
          if (res["data"]["editable"] != true) {
            ...
            axios
              .get(
                baseURL +
                  "writing/get-essay-writing-data/view/" +
                  id +
                  "/" +
                  userid,
                {
            ...
            axios
              .get(
                baseURL +
                  "writing/get-essay-writing-data/edit/" +
                  id +
                  "/" +
                  userid,
                {
  ...
  const handleSubmitComment = (data) => {
    ...
    axios
      .post(
        baseURL + "writing/submit-essay-comment/" + id + "/" + userid,
        {
          submitData: data,
          roomUserEmail: email,
        },
  ...
  const handleSubmitEditorButton = () => {
    ...
    axios
      .post(
        baseURL + "writing/define-essay-editor/" + id + "/" + userid,
        {
  ...
  return (
    <div>
      <PapperBlock
        title="EssayWriting Menu"
        whiteBg
        icon="ion-ios-grid-outline"
        desc="
            영어 에세이를 작성하고 공유해보세요. (에세이 편집에서는 사진 첨부 불가) "
      >
        <Fragment>
          <Grid
            container
          ...
          >
                ...
                <Grid item xs={12}>
                  ...
                  >
                    ...
                    >
                      ...
                      Created by&nbsp;-&nbsp;{metaDatas.owner_name}/{" "}
                      {metaDatas.owner_email}
                    </Typography>
                    <IconButton
                      onClick={editClicked}
                      ...
                      localStorage.getItem("email") == email && (
                        ...
                        >
                                    ...
                                    return (
                                      <FormControlLabel
                                        ...
                                        control={
                                          <Checkbox
                                            checked={
                                              dataState.friends[index][friend]
                                            }
                          ...
                          <DialogActions>
                            ...
                            <Button
                              onClick={handleSubmitEditorButton}
                              color="primary"
                            >
                              Ok
                            </Button>
            ...
            <Grid item xs={12}>
              {currentUserDataJson &&
                currentUserDataJson != undefined &&
                Object.keys(currentUserDataJson).length != 0 && (
                  <CommentSection
                    currentUser={currentUserDataJson}
                    commentData={
                      dataJsons != undefined &&
                      dataJsons &&
                      Object.keys(dataJsons).length != 0
                        ? dataJsons
                        : []
                    }
                    ...
                    onDeleteAction={(deleteData) => {
                      for (var i = 0; i < dataJsons.length; i++) {
                        if (
                          deleteData["parentOfDeleteId"] != undefined &&
                          deleteData["parentOfDeleteId"] ==
                            dataJsons[i]["comId"]
                        ) {
                          var x = dataJsons;
                          x.splice(i, 1);
                          ...
                          deleteData["parentOfDeleteId"] == undefined &&
                          dataJsons[i]["comId"] == deleteData["comIdToDelete"]
                        ) {
                          var x = dataJsons;
                          x.splice(i, 1);
                          setDataJsons(x);
